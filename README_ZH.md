...
- ✅ **修复多仓库配置丢失**: 从多个未配置的仓库安装时，仓库配置现在能够正确保存
- ✅ **修复 bash 补全**: Copilot 命令现在能正确显示 Copilot 指令而不是 Cursor 规则
- ✅ **改进 zsh 补全**: 添加了对嵌套子命令的支持，如 `ais cursor rules add` 和 `ais cursor plans add`
