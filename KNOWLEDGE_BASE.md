...
- **Issue**: When installing rules/plans/instructions from multiple unconfigured repositories in a single command, only the last repository configuration survives.
- **Root Cause**: The `repos` variable was captured before the loop but never updated after each `setConfig` call. Since `setConfig` performs a shallow merge, subsequent iterations used a stale snapshot of the repos object.
- **Fix**: Updated `installCursorRules`, `installCursorPlans`, and `installCopilotInstructions` functions to refresh the local `repos` variable after each `setConfig` call (lines 159, 234, 311 in src/index.ts).
- **Impact**: Multi-repo installs now correctly preserve all configured repositories.

### Bug #2: Bash completion returns wrong suggestions for copilot commands (MEDIUM)

- **Issue**: When typing `ais copilot add <TAB>`, bash completion shows cursor rules instead of copilot instructions.
- **Root Cause**: Bash completion script had a combined condition for both `cursor add` and `copilot add` commands, always calling `_complete cursor`.
- **Fix**: Split the combined condition into two separate bash conditionals (lines 1022-1031 in src/index.ts):
  - `ais cursor add <TAB>` now correctly calls `_complete cursor`
  - `ais copilot add <TAB>` now correctly calls `_complete copilot`
- **Impact**: Bash users now get correct completions for copilot commands.

### Bug #3: Zsh completion missing for nested cursor subcommands (LOW)

- **Issue**: Commands like `ais cursor rules add <TAB>` and `ais cursor plans add <TAB>` don't get name completion.
- **Root Cause**: The zsh completion script's `name` state handler was empty (only contained a comment), so 4-word commands with nested subcommands couldn't be completed.
- **Fix**: Populated the `name` state handler in zsh completion script (lines 1150-1199 in src/index.ts) to:
  - Handle `ais cursor add <TAB>` with cursor rules completion
  - Handle `ais cursor rules add <TAB>` with cursor rules completion
  - Handle `ais cursor plans add <TAB>` with cursor plans completion
  - Handle `ais copilot add <TAB>` with copilot instructions completion
- **Impact**: Zsh users now get consistent completion behavior for both 3-word and 4-word commands.

### Testing

- Configuration preservation tests verify the repos variable accumulation
- Shell completion script validation tests confirm separate conditions and populated name state
- Scenario documentation covers multi-repo installation flow without data loss
